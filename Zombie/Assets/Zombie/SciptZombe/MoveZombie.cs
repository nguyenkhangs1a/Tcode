...
public class MoveZombie : MonoBehaviour {
    ...
    [SerializeField]
    private GameObject BulletZB;
    [SerializeField]
    private Transform PointBulletZB;
   ...
   void Attack()
   {
	...
	void Update () {
       if(CreatZB.IsGameover  && _Player==null)
       {
           Debug.Log("return");
           isAttack = false;
               return;
       }
        else if(CreatZB.IsGameover==false)
       {
    ...
    void CreatBulletZB()
    {
       Destroy(Instantiate(BulletZB, PointBulletZB.position, Quaternion.identity),1f);
    }
}
